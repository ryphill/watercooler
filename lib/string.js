...
String.prototype.markdownCode = function () {
  // `System.out.println('Hello, world!')` => <span class="message-code">S.o.p("Hello, world!")</span>
  return this.replace(/`(.+?)`/g, '<span class="message-code">$1</span>');
}
...
String.prototype.createCages = function () {
  // inserts a randomly-sized Nic Cage (courtesy of placecage)
  var width = Math.floor(Math.random() * 200) + 150;
  var height = Math.floor(Math.random() * 200) + 150;

  return this.replace(/\s(\/cage)\s/g,
    '<img class="message-img" src="http://www.placecage.com/' + width + '/' + height + '" />');
}

String.prototype.createCarlton = function () {
  // inserts a Carlton dancing (courtesy of tumblr)

  return this.replace(/\s(\/carlton)\s/g,
    '<img class="message-img" src="http://25.media.tumblr.com/tumblr_lt4djiyO7k1qbymhco1_r4_250.gif" />');
}
